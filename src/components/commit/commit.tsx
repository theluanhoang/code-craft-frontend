...
const CommitGraph: React.FC = () => {
  ...
  const handleBranch = () => {
    if (head) {
      const positionAvailable = handleCommitAtPosition(
        head.x + 100,
        head.y + 100
      )
  ...
  const hasCommitAtPosition = (x: number, y: number) => {
    ...
    const filteredDimension = dimensionCommits.filter(
      (dimensionCommit) =>
        position.x === dimensionCommit.x && position.y === dimensionCommit.y
    )
          ...
          <Layer>
            ...
            {circles.map((circle: ICircle) => (
              ...
              >
                ...
                {commitPerBranch[circle.branch] &&
                  commitPerBranch[circle.branch].latestCommit === circle && (
                    <TagComponent
